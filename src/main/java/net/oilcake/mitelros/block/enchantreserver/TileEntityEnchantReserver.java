...
public class TileEntityEnchantReserver extends TileEntity implements ISidedInventory {
  ...
  private final EnchantReserverSlots slots = new EnchantReserverSlots((IInventory)this);
    ...
    if (!(getWorldObj()).isRemote) {
      ...
      if (inputStack != null)
        if (inputStack.itemID == Item.diamond.itemID && inputStack.stackSize * 500 + getEXP() <= getMAXEXP()) {
          ...
          this.EXP += 50 * size;
          this.slots.getInPut().putStack(null);
        } else if (inputStack.itemID == Items.lapis.itemID && inputStack.stackSize * 25 + getEXP() <= getMAXEXP()) {
          int size = inputStack.stackSize;
          this.EXP += 25 * size;
          this.slots.getInPut().putStack(null);
        }  
      ...
      if (outputStack != null) {
        if (getEXP() >= 200 && 
          outputStack.itemID == Item.potion.itemID && outputStack.stackSize * 200 <= getEXP() - 2000) {
          this.EXP -= 200 * outputStack.stackSize;
          this.slots.getOutPut().putStack(Item.expBottle.getItemStackForStatsIcon());
        } 
        if (getEXP() >= 5 && 
          outputStack.itemID == Item.copperNugget.itemID && outputStack.stackSize * 5 <= getEXP() - 2000) {
          this.EXP -= 5 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Item.coinCopper, outputStack.stackSize));
        } 
        if (getEXP() >= 25 && 
          outputStack.itemID == Item.silverNugget.itemID && outputStack.stackSize * 25 <= getEXP() - 2000) {
          this.EXP -= 25 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Item.coinSilver, outputStack.stackSize));
        } 
        if (getEXP() >= 50 && 
          outputStack.itemID == Items.nickelNugget.itemID && outputStack.stackSize * 50 <= getEXP() - 2000) {
          this.EXP -= 50 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Items.nickelCoin, outputStack.stackSize));
        } 
        if (getEXP() >= 100 && 
          outputStack.itemID == Item.goldNugget.itemID && outputStack.stackSize * 100 <= getEXP() - 2000) {
          this.EXP -= 100 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Item.coinGold, outputStack.stackSize));
        } 
        if (getEXP() >= 500 && 
          outputStack.itemID == Item.ancientMetalNugget.itemID && outputStack.stackSize * 500 <= getEXP() - 2000) {
          this.EXP -= 500 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Item.coinAncientMetal, outputStack.stackSize));
        } 
        if (getEXP() >= 2500 && 
          outputStack.itemID == Item.mithrilNugget.itemID && outputStack.stackSize * 2500 <= getEXP() - 2000) {
          this.EXP -= 2500 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Item.coinMithril, outputStack.stackSize));
        } 
        if (getEXP() >= 5000 && 
          outputStack.itemID == Items.tungstenNugget.itemID && outputStack.stackSize * 5000 <= getEXP() - 2000) {
          this.EXP -= 5000 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Items.tungstenCoin, outputStack.stackSize));
        } 
        if (getEXP() >= 10000 && 
          outputStack.itemID == Item.adamantiumNugget.itemID && outputStack.stackSize * 10000 <= getEXP() - 2000) {
          this.EXP -= 10000 * outputStack.stackSize;
          this.slots.getOutPut().putStack(new ItemStack((Item)Item.coinAdamantium, outputStack.stackSize));
        } 
      } 
    } 
  }
  ...
  public void destroyInventory() {
    Arrays.fill((Object[])this.items, (Object)null);
  }
}
