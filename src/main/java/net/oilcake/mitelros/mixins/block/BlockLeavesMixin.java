...
public class BlockLeavesMixin extends BlockLeavesBase {
    ...
    public boolean updateTick(World par1World, int par2, int par3, int par4, Random par5Random) {
        ...
        if ((var6 & 8) != 0 && (var6 & 4) == 0) {
            ...
            if (RNG.chance_in_100[++RNG.random_number_index + (int) par1World.total_time & 32767] && !wasPlaced(var6) && (!par1World.getAsWorldServer().fast_forwarding || RNG.chance_in_32[++RNG.random_number_index & 32767]) && par5Random.nextInt(((ITFWorld) par1World).getWorldSeason() == 2 ? 200 : 500) == 0) {
    ...
    public int dropBlockAsEntityItem(BlockBreakInfo info) {
        ...
        if (info.getBiome().isSwampBiome() && info.world.rand.nextInt(2) == 0)
            return 0;
        int leaf_kind = getBlockSubtype(info.getMetadata());
        int num_drops;
        if ((num_drops = dropBlockAsEntityItem(info, Block.sapling.blockID, leaf_kind, 1, 0.01F)) > 0)
            return num_drops;
        if ((num_drops = dropBlockAsEntityItem(info, Item.stick.itemID, 0, 1, 0.05F)) > 0)
            return num_drops;
        if (leaf_kind == 0)
            return dropBlockAsEntityItem(info, info.getBiome().isJungleBiome() ? Item.orange.itemID : Item.appleRed.itemID, 0, 1, 0.005F);
        if (info.world.rand.nextInt(2) > 0)
            return (leaf_kind == 4) ? dropBlockAsEntityItem(info, Items.lemon.itemID, 0, 1, 0.005F) : 0;
