...
public class DamageMixin {
    ...
    protected float applyTargetDefenseModifiers(EntityLivingBase target, EntityDamageResult result) {
        ...
        if (target instanceof EntityPlayer && effective_protection >= this.amount) {
            ...
            return Math.max(this.amount - effective_protection, ((Boolean) StuckTagConfig.TagConfig.TagArmament.ConfigValue).booleanValue() ? 0.0F : 1.0F);
        }
        return Math.max(this.amount - effective_protection, ((Boolean) StuckTagConfig.TagConfig.TagInstinctSurvival.ConfigValue).booleanValue() ? 0.0F : 1.0F);
    }
