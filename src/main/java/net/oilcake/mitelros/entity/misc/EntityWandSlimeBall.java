...
public class EntityWandSlimeBall extends EntityAbstractWandShoot {
    ...
    protected void onImpact(RaycastCollision rc) {
        if (!this.worldObj.isRemote)
            if (rc.isEntity() && rc.getEntityHit() instanceof EntityLivingBase) {
                ...
                for (int i = 0; i < 32; i++)
                    this.worldObj.spawnParticle(EnumParticle.slime, this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D);
            } else {
